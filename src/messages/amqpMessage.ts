...
class AmqpMessage {
  private message: amqp.ConsumeMessage | null | undefined;

  private content: Record<string, unknown> | string;

  private channel: amqp.Channel | amqp.ConfirmChannel | undefined;
  ...
  fromApi = (): boolean => {
    return !!(
      this.message?.properties.headers && this.message?.properties.headers[AmqpMessage.FROM_API_HEADER] === 'true'
    );
  };
