...
var StompStreamClient = function (_StompClient) {
  ...
  _createClass(StompStreamClient, [{
    ...
    value: function unsubscribe(deviceId, channel) {
      ...
      return new _bluebird2.default(function (resolve, reject) {
        if (_this3._stompConnection === undefined) {
          ...
          var topic = _this3._streamChannelTopicFor(deviceId, channel);
    ...
    value: function unsubscribeStream(stream) {
      var _this4 = this;
      ...
      return new _bluebird2.default(function (resolve, reject) {
        if (_this4._stompConnection === undefined) {
          ...
          var topic = _this4._streamTopicFor(stream);
          var subscription = _this4._subscriptions[topic];
          subscription.unsubscribe(topic);
          delete _this4._subscriptions[topic];
          resolve(true);
        }
      });
    }
    ...
    value: function disconnect() {
      ...
      return new _bluebird2.default(function (resolve, reject) {
        if (_this5._stompConnection === undefined) {
          ...
          for (var subscription in _this5._subscriptions) {
            if (subscription) {
              _this5._subscriptions[subscription].unsubscribe();
            }
          }
          _this5._subscriptions = {};
          _this5._stompConnection.disconnect(function () {
            _this5._stompConnection = undefined;
            resolve(true);
    ...
    value: function _attachStreamHook(streamHook, opts) {
      ...
      return new _bluebird2.default(function (resolve, reject) {
        ...
        _this6.connect().then(function (client) {
          var topic = void 0;
          var tempQueue = void 0;
          if (stream) {
            if (!_this6.liveStreamExists(stream)) {
            ...
            if (cache) {
              ...
              topic = _this6._cachedStreamTopicFor(stream);
            } else {
              // Uncached streams are connected to the stream exchange and create a temp queue
              topic = _this6._streamTopicFor(stream);
              tempQueue = _this6.tempQueue(stream, _this6._liveStreamSuffix);
            }
          } else {
            ...
            topic = _this6._streamChannelTopicFor(deviceId, channel);
            tempQueue = _this6.tempQueue(deviceId, channel);
          }
          ...
          var messageCallback = function messageCallback(message) {
            callback(_this6._parseContent(message.body), message.headers);
          };
          var subscription = client.subscribe(topic, messageCallback, subscriptionHeaders);
          _this6._subscriptions[topic] = subscription;
          resolve(true);
        }).catch(function (reason) {
          reject(reason);
        });
