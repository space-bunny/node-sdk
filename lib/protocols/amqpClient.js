    ...
    value: function publish(channel, message) {
      ...
      return new _bluebird2.default(function (resolve, reject) {
        _this3._createChannel('output', opts).then(function (ch) {
          var bufferedMessage = Buffer.from(_this3._encapsulateContent(message));
          var _opts = opts,
          ...
          var promises = [ch.checkExchange(_this3.deviceId()), ch.publish(_this3.deviceId(), _this3._routingKeyFor({ channel: channel, routingKey: routingKey, topic: topic }), bufferedMessage, opts)];
