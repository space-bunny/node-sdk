...
var AmqpClient = function (_SpaceBunny) {
  ...
  function AmqpClient() {
    ...
    var _this = _possibleConstructorReturn(this, (AmqpClient.__proto__ || Object.getPrototypeOf(AmqpClient)).call(this, opts));

    _this._amqpConnection = undefined;
    _this._amqpChannels = {};
    var amqpOptions = CONFIG.amqp;
    _this._protocol = amqpOptions.protocol;
    _this._tlsProtocol = amqpOptions.tls.protocol;
    ...
    _this._connectionOpts = amqpOptions.connectionOpts;
    return _this;
  }
  ...
  _createClass(AmqpClient, [{
    ...
    value: function onReceive(callback) {
      ...
      return new _bluebird2.default(function (resolve, reject) {
        ...
        localOpts = (0, _merge3.default)((0, _cloneDeep3.default)(_this2._subscribeArgs), localOpts);
        localOpts.noAck = localOpts.ack === null;
        _this2._createChannel('input', localOpts).then(function (ch) {
          return _bluebird2.default.all([ch.checkQueue(_this2.deviceId() + '.' + _this2._inboxTopic, _this2._inputQueueArgs), ch.consume(_this2.deviceId() + '.' + _this2._inboxTopic, function (message) {
            // Create message object
            var amqpMessage = new _amqpMessage2.default(message, _this2._deviceId, localOpts);
            var ackNeeded = _this2._autoAck(localOpts.ack);
            // Check if should be accepted or not
            if (amqpMessage.blackListed()) {
              if (ackNeeded) {
                ch.nack(message, localOpts.allUpTo, localOpts.requeue);
              }
              return;
            }
            ...
            if (ackNeeded) {
              ch.ack(message, localOpts.allUpTo);
            }
          }, localOpts)]);
        }).then(function (res) {
          resolve(res);
    ...
    value: function publish(channel, message) {
      ...
      return new _bluebird2.default(function (resolve, reject) {
        var localOpts = (0, _cloneDeep3.default)(opts);
        localOpts = (0, _merge3.default)((0, _cloneDeep3.default)(_this3._publishArgs), localOpts);
        _this3._createChannel('output', localOpts).then(function (ch) {
          ...
          var promises = [ch.checkExchange(_this3.deviceId()), ch.publish(_this3.deviceId(), _this3._routingKeyFor({ channel: channel, routingKey: routingKey, topic: topic }), Buffer.from(_this3._encapsulateContent(message)), localOpts)];
    ...
    value: function connect() {
      ...
      return new _bluebird2.default(function (resolve, reject) {
        var connectionOpts = (0, _cloneDeep3.default)(opts);
        connectionOpts = (0, _merge3.default)((0, _cloneDeep3.default)(_this5._connectionOpts), connectionOpts);
        _this5.getEndpointConfigs().then(function (endpointConfigs) {
          ...
          if (_this5.isConnected()) {
            ...
            if (_this5._tls) {
              connectionString = _this5._tlsProtocol + '://' + (connectionParams.deviceId || connectionParams.client) + ':' + (connectionParams.secret + '@' + connectionParams.host + ':') + (connectionParams.protocols.amqp.tlsPort + '/' + connectionParams.vhost.replace('/', '%2f'));
              connectionOpts = (0, _merge3.default)(connectionOpts, _this5._tlsOpts);
            } else {
            ...
            return _amqplib2.default.connect(connectionString, connectionOpts).then(function (conn) {
    ...
    value: function _createChannel(channelName) {
      ...
      return new _bluebird2.default(function (resolve, reject) {
        if (_this6._amqpChannels[channelName]) {
          ...
          _this6.connect().then(function (conn) {
            ...
            _this6._amqpChannels[channelName].on('error', function (err) {
              console.error(err);
              _this6._amqpChannels[channelName] = undefined;
            });
            _this6._amqpChannels[channelName].on('close', function (err) {
              console.error(err);
              _this6._amqpChannels[channelName] = undefined;
            });
            resolve(ch);
    ...
    key: '_routingKeyFor',
    value: function _routingKeyFor(params) {
      var _params$channel = params.channel,
      ...
      if (routingKey) {
        ...
        var messageRoutingKey = this.deviceId();
